...
echo $PATH|tee /tmp/tee.log|tr '[a-z]' '[A-Z]'
</pre>
...
<pre>
    ...
    zcat：不显示展开的前提下查看文件内容
        ...
        解压：zact DEST.gz > SRC
        备份数据库时候常用：mysqldump  -p123456 -B  -F -R --mast-date=1 --events test|gzip > test.sql.gz
	...
	7za 
		a：添加新的压缩文档  7za a new.7a  file
		l：列出压缩文档	   7za l new.7a  
		u：更新压缩文档     7za u new.7a log.txt
		d：删除压缩文档内容  7za d new.7a log.txt  
		x：解压压缩文档     7za x new.7a -o/tmp/ 注意o直接没有空格
